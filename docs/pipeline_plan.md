...
Sample data: `data/PSY582-S26-WK9-Thu.wav` (formerly `geisler.wav`, the verbatim reference) and
`data/PSY777-F26-WK1-Wed.m4a` (see `docs/environments.md`). The old output `transcripts/lecture transcript.txt` — fully
...
| **Diarization speaker count — and time — grow with duration** | **Measured on full lectures 2026-09-02.** community-1 on 79 min: 4 labels, 422 turns, ~10 min of wall time; on 94 min: 2 labels, 670 turns; on **145 min: 13 labels, 1,641 turns, 31 minutes of wall time at 0% GPU** — one label holds 99 of the 145 minutes and twelve share the rest. The tail of small labels is fragmentation, not twelve people. The render-time speaker map can fold them, but thirteen is a lot to map by hand. **The time was not clustering (2026-09-22):** handed a path, pyannote 4.0.3 opens a fresh decoder for every chunk its embedding step crops, ~340 ms each on a 48 kHz WAV, one per second of audio. Passing a decoded waveform instead made the same two 145 min lectures diarize in **1.9 min** instead of ~32, with every turn identical to the path run. Next small feature: a `--max_speakers` passthrough to the pipeline (pyannote accepts `num_speakers` / `min_speakers` / `max_speakers` at call time — verify on 4.0.3 before wiring, D19), which bounds the label count. Still unverified against the audio. |
